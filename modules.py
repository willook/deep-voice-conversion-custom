...
def conv1d(inputs,
    ...
    with tf.variable_scope(scope):
        ...
        conv_layer = tf.keras.layers.Conv1D(**params)
...
def gru(inputs, num_units=None, bidirection=False, seqlens=None, scope="gru", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        GruCell = tf.contrib.cudnn_rnn.CudnnGRU(2, num_units,dropout, direction = 'bidirectional')
        # cell = tf.nn.rnn.DropoutWrapper(cell,output_keep_prob = 0.8)
        outputs, _ = GruCell(inputs)
        return tf.concat(outputs, 2)  
        
        # if bidirection: 
            # cell_bw = tf.contrib.rnn.GRUCell(num_units)
            # cell_bw = tf.contrib.cudnn_rnn.CudnnCompatibleGRUCell(num_units)
            #cell_bw = tf.contrib.cudnn_rnn.CudnnGRU(2, num_units,dropout=0.2, direction = 'bidirection')
            # cell_bw = tf.nn.rnn.DropoutWrapper(cell_bw, output_keep_prob = 0.8)
...
def cnn(input, num_banks, hidden_units, norm_type='bn', is_training=True):    
