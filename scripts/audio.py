...
def spec2wavfaster(mag, n_fft, win_length, hop_length, num_iters=30, phase=None):
    
    assert (num_iters > 0)
    
    if phase is None:
        cn_angle = np.pi * np.random.rand(*mag.shape)
    else:
        cn_angle = phase

    s = mag
    cn = s*np.exp(1.j*c0_angle)
    x = None

    for i in range(num_iters):
        x = librosa.istft(cn, win_length=win_length, hop_length=hop_length)
        if i is not num_iters-1:
            cn = librosa.stft(x, n_fft=n_fft, win_length=win_length, hop_length=hop_length)
            _, cn_angle = librosa.magphase(cn)
            cn_angle = np.angle(cn_angle)
            cn = s*np.exp(1.j*cn_angle)
