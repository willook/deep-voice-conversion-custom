...
import numpy as np
from audio import spec2wav_lws, inv_preemphasis, db2amp, denormalize_db
import datetime
...
def convert(predictor, mfcc, spec, mel_spec):
    ...
    audio = []
    # print('pred_spec.shape : ', pred_spec.shape)
    # print('pred_spec[0].shape : ', pred_spec[0].shape)
    # print('pred_spec.dtype : ', pred_spec.dtype)
    ...
    print("Converting Spectrogram-to-Wave time:{}s".format(convert_time.seconds))
...
def do_convert(args, logdir1, logdir2):
    ...
    while True:
        ...
        mfcc, spec, mel_spec = get_mfcc_and_spectrogram(input_name, trim=False, isConverting=True)
        output_audio, ppgs = convert(predictor, mfcc, spec, mel_spec)

        # Saving voice-converted audio to 32-bit float wav file
        # print(audio.dtype)
        output_audio = output_audio.astype(np.float32)
        write_wav(path="./converted/"+input_name,y=output_audio[0],sr=hp.default.sr)
